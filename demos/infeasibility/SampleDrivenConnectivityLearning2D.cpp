...
using namespace std::placeholders;
 ...
 bool isStateValid(const ob::State *state)
 {
    // a narrow passage 2d environment.
     ...
     if (dist_square < 1 || dist_square > 4) return true;
     else if (x > 0 and abs(y) < passage_width) return true; // on narrow passage
     else return false;
   }

// SDCL sampler
ob::ValidStateSamplerPtr allocSDCLValidStateSampler(
    const ob::SpaceInformation* si, const ob::PlannerPtr planner) {
...
bool solve(double duration) {
  ...
  ob::ValidStateSamplerPtr sampler(
      std::make_shared<ob::SDCLValidStateSampler>(si.get(), planner_));

  auto allocSDCLValidStateSampler_partial =
      [&](const ob::SpaceInformation* si) { return sampler; };
  ...
  // get the SDCL sampler, must setup the planner first, the sampler pointer are
  // changed during planner setup. everytime this is called, detele is called.
  // so we should not use this. ob::SDCLValidStateSampler* sdcl_vss =
  // dynamic_cast<ob::SDCLValidStateSampler*>(si->allocValidStateSampler().get());

  // setup a seperate thread for generating narrow passage samples.
  // std::thread sdclThread([&] {sdcl_vss->generateSDCLSamples();});
  // std::thread sdclThread(&ob::SDCLValidStateSampler::generateSDCLSamples,
  // sdcl_vss); std::cout << "SDCL thread starting ..." << std::endl;
  // sdcl_vss->startSDCLThread();
  ...
  // auto sdcl_vss = dynamic_cast<ob::SDCLValidStateSampler*>(sampler.get());
  // sdcl_vss->endSDCLThread();
...
void parse(int argc, char** argv, double* duration) {
  if (argc != 2) {
    std::cout << "Please provide a duration(required). "
              << std::endl;
    exit(1);
  } else {
